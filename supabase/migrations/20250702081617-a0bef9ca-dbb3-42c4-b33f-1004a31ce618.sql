...
INSERT INTO public.prompt_templates (title, content, category, is_public) VALUES
('Schedule Social Posts', 'Create a workflow to schedule social media posts from a Google Sheet', 'Social Media', true),
('Monitor Brand Mentions', 'Track brand mentions on Twitter and summarize daily', 'Monitoring', true),
('Expense Approval', 'Set up an expense approval workflow with manager notifications', 'Business', true),
('Email Classification', 'Automatically classify incoming emails by priority and category', 'Email', true),
('Data Backup', 'Create automated daily backups of important data sources', 'Automation', true);

-- Insert some default agents
INSERT INTO public.agents (user_id, name, type, description, configuration) VALUES
((SELECT id FROM auth.users LIMIT 1), 'Text Summarizer', 'text_summarizer', 'Condenses long texts into concise summaries', '{"max_length": 200, "style": "bullet_points"}'),
((SELECT id FROM auth.users LIMIT 1), 'Data Extractor', 'data_extractor', 'Pulls structured data from emails, PDFs and more', '{"output_format": "json", "fields": ["name", "email", "phone"]}'),
((SELECT id FROM auth.users LIMIT 1), 'Research Assistant', 'research_assistant', 'Searches and compiles information on a topic', '{"sources": ["web", "academic"], "depth": "comprehensive"}');
